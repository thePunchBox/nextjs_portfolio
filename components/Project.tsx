...
export function Project({ title, description, tags, imageUrl }: ProjectProps) {
	...
	return (
		...
		>
			<section
				className=' bg-gray-100 max-w-[42rem] borderBlack rounded-lg overflow-hidden sm:pr-8 relative sm:h-[20rem] hover:bg-gray-200 transition group-even:pl-8 dark:text-white dark:bg-white/10 dark:hover:bg-white/20'
			>
				<div className='pt-4 pb-7 px-5 sm:pl-10 sm:pr-2 sm:pt-10 sm:max-w-[50%] flex flex-col h-full group-even:ml-[18rem]'>
					<h3 className='text-2xl font-semibold'>{title}</h3>
					<p className='mt-2 landing-relaxed text-gray-700 dark:text-white/70'>{description}</p>
